...
class Galeri extends CI_Controller {
    ...
    {
        $this->load->view('backend/cms/galeri');
    }

    public function datatables()
    {
        $list = $this->GaleriModel->get_datatables();
        ...
        foreach ($list as $apps) {
            $no++;
            $row = array();
            $row[] = $apps->galeri_title;
            $row[] = $apps->galeri_slug;
            $row[] = substr($apps->galeri_content, 0, 50);
            $row[] = $apps->category;
            $row[] = $apps->post_by;
			$row[] = $apps->created_on;
            if ($apps->image) {
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->GaleriModel->count_all(),
                        "recordsFiltered" => $this->GaleriModel->count_filtered(),
                        "data" => $data,
                );
    ...
    {
        $data = $this->GaleriModel->get_by_id($id);
    ...
    public function store()
    {
        ...
        $insert = $this->GaleriModel->save($data);
    ...
    public function update()
    {
        ...
        $this->GaleriModel->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function destroy($id)
    {
        ...
        // $this->GaleriModel->delete_by_id($id);
