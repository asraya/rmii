                ...
                <div class="panel-header bg-primary-gradient">
                    <div class="page-inner py-5">
                        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                            <div>
                                <h2 class="text-white pb-2 fw-bold">galeri Page </h2>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex align-items-center">
                                    <h4 class="card-title">Tables of galeri</h4>
...
$(document).ready(function() {
        ...
        table = $('#show_table').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('backend/galeri/datatables')?>",
                "type": "POST"
            },
    ...
    function ajax_save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('backend/galeri/store')?>";
        } else {
            url = "<?php echo site_url('backend/galeri/update')?>";
        }
    ...
    function ajax_edit(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('backend/galeri/edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                ...
                $('[name="id"]').val(data.id);
                $('[name="galeri_title"]').val(data.galeri_title);
                $('[name="galeri_slug"]').val(data.galeri_slug);
                $('[name="galeri_content"]').val(data.galeri_content);                
                $('[name="category"]').val(data.category);
                $('[name="image"]').val(data.image);
                $('[name="post_by"]').val(data.post_by);
				$('[name="created_on"]').val(data.created_on);
        ...
        iziToast.question({
            ...
            buttons: [
                ['<button><b>YES</b></button>', function (instance, toast) {
                    $.ajax({
                        url : "<?php echo site_url('backend/galeri/destroy')?>/"+id,
                        type: "POST",
                        dataType: "JSON",
                        success: function(data)
                        {
